...
if(isset($_POST['submit_testimonial'])) {
    ...
    // Check if they already submitted one (optional: to update instead of insert)
        ...
        <aside id="sidebarMenu" class="w-72 bg-white border-r border-slate-100 flex flex-col fixed inset-y-0 left-0 -translate-x-full lg:translate-x-0 lg:static h-screen z-50 transition-transform duration-300 ease-in-out">
            ...
            <nav class="flex-1 pt-8 px-4 space-y-2 overflow-y-auto">
                ...
                <a href="student_resources.php"
                    class="flex items-center gap-3.5 px-6 py-4 text-slate-600 hover:bg-slate-50 rounded-xl font-semibold transition-all group">
            ...
            <div class="p-6 border-t border-slate-100">
                <button onclick="confirmLogout()"
                    class="w-full flex items-center gap-3 px-6 py-4 text-red-500 hover:bg-red-50 rounded-xl font-bold transition-all">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
        ...
        <main class="flex-1 min-w-0 w-full">
            ...
            <div class="p-4 sm:p-10 max-w-[1500px] mx-auto space-y-10">
                ...
                <div
                    ...
                    <a href="enroll.php"
                        class="inline-flex items-center gap-3 px-10 py-5 bg-indigo-600 text-white font-bold rounded-xl hover:bg-slate-950 transition-all group">
                        <span>Submit Enrollment</span>
                        <svg class="w-5 h-5 group-hover:translate-x-1.5 transition" fill="none" stroke="currentColor"
                            viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                ...
                <div
                    ...
                    <div class="space-y-3 w-full lg:w-auto">
                        <div>
                            ...
                            <div class="flex flex-wrap items-center gap-3 mt-3">
                                <span
                                    class="text-sm font-semibold text-slate-600 flex items-center gap-2 bg-slate-50 px-3 py-1.5 rounded-lg border border-slate-100">
                                    <span class="text-indigo-500">📚</span> <?= $enroll['program_type'] ?>
                                </span>
                                <span
                                    class="text-sm font-semibold text-slate-600 flex items-center gap-2 bg-slate-50 px-3 py-1.5 rounded-lg border border-slate-100">
                                    <span class="text-indigo-500">📍</span>
                                    <?= $enroll['enrolled_at'] ?: 'Not Specified' ?>
                                </span>
                                <?php if($enroll['insured'] == 1): ?>
                                <span
                                    class="text-[10px] font-black uppercase tracking-wider bg-emerald-50 text-emerald-600 px-3 py-1.5 rounded-lg border border-emerald-100 flex items-center gap-1.5">
                                    <svg class="w-3.5 h-3.5" fill="currentColor" viewBox="0 0 20 20">
                                        <path fill-rule="evenodd"
                                            d="M2.166 4.999A11.954 11.954 0 0010 1.944 11.954 11.954 0 0017.834 5c.11.65.166 1.32.166 2.001 0 5.225-3.34 9.67-8 11.317C5.34 16.67 2 12.225 2 7c0-.682.057-1.35.166-2.001zM9 10a1 1 0 011-1h3a1 1 0 110 2H10a1 1 0 01-1-1z"
                                            clip-rule="evenodd" />
                                    </svg>
                                    Insured
                                </span>
                                <?php else: ?>
                                <span
                                    class="text-[10px] font-black uppercase tracking-wider bg-slate-50 text-slate-400 px-3 py-1.5 rounded-lg border border-slate-100">
                                    No Insurance
                                </span>
                                <?php endif; ?>
                            </div>
                    ...
                    <div class="flex items-center gap-4 w-full lg:w-auto">
                        <div
                            ...
                            <span
                                class="text-sm font-black uppercase text-indigo-700 tracking-tighter"><?= $enroll['status'] ?></span>
                        </div>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                    ...
                    <div
                        class="bg-white p-7 rounded-3xl border border-slate-100 shadow-lg shadow-slate-100/50 stat-card flex items-center gap-5 hover:border-emerald-100">
                        <div
                        ...
                        <div>
                            ...
                            <h3 class="text-2xl font-extrabold text-emerald-600"><?= number_format($total_paid, 2) ?>
                            </h3>
                        </div>
                    </div>
                    <div
                        class="bg-white p-7 rounded-3xl border border-slate-100 shadow-lg shadow-slate-100/50 stat-card flex items-center gap-5 hover:border-rose-100">
                        <div
                            class="w-14 h-14 rounded-2xl bg-rose-50 text-rose-500 flex items-center justify-center text-2xl font-black">
                            !</div>
                        <div>
                    ...
                    <div
                        class="bg-indigo-600 p-7 rounded-3xl border border-indigo-600 shadow-lg shadow-indigo-100 stat-card flex items-center gap-5">
                        <div
                            class="w-14 h-14 rounded-2xl bg-white/10 text-indigo-100 flex items-center justify-center text-xl font-black">
                            #</div>
                        <div>
                ...
                <div class="bg-white p-6 sm:p-8 rounded-[2rem] shadow-sm border border-slate-100">
                    ...
                    <form action="" method="POST">
                        <textarea name="testimonial_content" required
                            class="w-full p-4 bg-slate-50 border border-slate-100 rounded-2xl focus:ring-4 focus:ring-blue-500/5 outline-none text-sm min-h-[120px]"
                            placeholder="How was your experience with C-Familia?"></textarea>
                        <button type="submit" name="submit_testimonial"
                            class="mt-4 bg-blue-600 text-white px-6 py-3 rounded-xl font-bold text-sm hover:bg-blue-700 transition">Submit
                            Review</button>
                    </form>
                </div>
                    ...
                    <div class="lg:col-span-1 space-y-6">
                        <div class="bg-white rounded-3xl border border-slate-100 shadow-lg shadow-slate-100/50">
                            <div class="px-8 py-6 border-b border-slate-100">
                                <h3 class="font-extrabold text-slate-950 text-sm tracking-tight uppercase">Latest News
                                </h3>
                            </div>
                            <div class="p-8 space-y-6">
                                ...
                                <div class="space-y-2">
                                    <span
                                        class="text-[10px] font-extrabold text-indigo-600 uppercase"><?= date('F d, Y', strtotime($ann['created_at'])) ?></span>
                                    <h4 class="font-bold text-slate-900 leading-snug"><?= $ann['title'] ?></h4>
                                    <p class="text-xs text-slate-500 line-clamp-2"><?= $ann['message'] ?></p>
                                </div>
                                <?php endwhile; ?>
                                <?php else: ?>
                                <p class="text-center py-4 text-xs font-bold text-slate-400 italic">No announcements.
                                </p>
                                <?php endif; ?>
                            </div>
                        </div>
                                    ...
                                    <tbody class="divide-y divide-slate-100">
                                        ...
                                        <tr class="hover:bg-slate-50/50 transition-colors">
                                            ...
                                            <td class="px-6 sm:px-10 py-6 text-right">
                                                ...
                                                ?>
                                                <span
                                                    class="inline-block px-3 py-1.5 <?= $cl ?> text-[9px] font-extrabold rounded-lg border uppercase tracking-wider"><?= $st ?></span>
                                            </td>
                                        </tr>
                                        <?php endwhile; ?>
                                        <?php else: ?>
                                        <tr>
                                            <td colspan="3"
                                                class="p-20 text-center text-xs text-slate-300 font-extrabold italic">No
                                                payment history.</td>
                                        </tr>
                                        <?php endif; ?>
                                    </tbody>
