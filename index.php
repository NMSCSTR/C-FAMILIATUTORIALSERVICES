...
// This assumes 'status' in enrollments or a similar check. 
// For now, let's count total passers vs a target or total students.
...
$display_rate = ($total_passers > 0) ? "95%" : "0%"; // You can adjust this logic later
?>
<!DOCTYPE html>
<html lang="en">
...
<nav class="sticky top-0 z-50 bg-white/90 backdrop-blur-md border-b border-slate-200 px-6 py-4">
    ...
    <header class="relative bg-slate-900 py-24 lg:py-32 overflow-hidden">
        ...
        <div class="relative max-w-7xl mx-auto px-6 text-center lg:text-left grid lg:grid-cols-2 items-center gap-12">
            <div>
                <div
                    class="inline-flex items-center gap-3 px-4 py-2 rounded-2xl bg-white/5 border border-white/10 mb-8 animate-bounce-slow">
                    <div
                        class="w-10 h-10 bg-blue-600 rounded-xl flex items-center justify-center font-black text-xl shadow-lg shadow-blue-600/40 text-white">
                        C</div>
                ...
                <div class="flex flex-wrap justify-center lg:justify-start gap-4">
                    <a href="register.php"
                        class="px-8 py-4 bg-blue-600 text-white rounded-xl text-lg font-bold hover:bg-blue-500 transition-all transform hover:scale-105">Enroll
                        Now</a>
                    <a href="#passers"
                        class="px-8 py-4 bg-white/10 text-white border border-white/20 rounded-xl text-lg font-bold hover:bg-white/20 transition-all">View
                        Success Stories</a>
                </div>
            </div>
            ...
            <div class="hidden lg:block">
                <div
                    class="bg-white/10 backdrop-blur-lg p-8 rounded-3xl border border-white/20 relative overflow-hidden group">
                    <div
                        class="absolute -right-10 -top-10 w-32 h-32 bg-blue-500/20 rounded-full blur-3xl group-hover:bg-blue-500/40 transition-all">
                    </div>

                    <div class="flex items-center gap-4 mb-6 relative">
                        <div
                            class="w-12 h-12 bg-green-500 rounded-full flex items-center justify-center text-white font-bold shadow-lg shadow-green-500/20">
                            ✓</div>
                        <p class="text-white text-lg font-semibold"><?= $display_rate ?> Passing Rate
                            (<?= $total_passers ?>+ Passers)</p>
                    </div>
                    <div class="space-y-4 relative">
                        ...
                        <p class="text-slate-400 text-xs font-medium tracking-wide">Based on 2025 Licensure Results</p>
                    </div>
                </div>
            </div>
        </div>
    ...
    <section id="announcements" class="py-16 px-6 max-w-7xl mx-auto">
        ...
        <div class="grid md:grid-cols-3 gap-6">
            ...
            ?>
            <div
                class="p-6 bg-white border border-slate-200 rounded-2xl shadow-sm hover:shadow-md transition relative overflow-hidden">
                <?php if($is_urgent): ?>
                <span
                    class="absolute top-0 right-0 bg-red-500 text-white text-[10px] px-3 py-1 font-bold uppercase tracking-widest">Urgent</span>
                <?php endif; ?>
    ...
    <section id="posts" class="py-16 px-6 max-w-7xl mx-auto border-t border-slate-100">
        ...
        <div class="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
            ...
            ?>
            <article
                class="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden hover:shadow-xl transition-shadow duration-300">
                <div class="p-6">
                    <div class="flex items-center gap-2 mb-3">
                        <span
                            class="px-2 py-1 bg-indigo-50 text-indigo-600 text-[10px] font-bold rounded uppercase">Resource</span>
                    </div>
                    <h4 class="text-xl font-bold mb-2"><?= $post['title'] ?></h4>
                    <p class="text-slate-600 text-sm mb-4 line-clamp-2"><?= $post['content'] ?></p>

                    <?php if($post['file_path']): ?>
                    <a href="uploads/resources/<?= $post['file_path'] ?>"
                        class="text-blue-600 text-sm font-bold hover:underline flex items-center gap-2">
                        Download Attachment →
                    </a>
                    <?php endif; ?>
                </div>
    ...
    <section id="passers" class="py-20 bg-slate-100 px-6">
        ...
        <div class="max-w-7xl mx-auto grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-6 text-center">
            ...
            <div class="p-5 bg-white rounded-2xl shadow-sm border border-slate-200 group">
                <img src="uploads/passers/<?= $passer['photo'] ?>"
                    class="w-20 h-20 rounded-full mx-auto mb-3 object-cover border-4 border-slate-50 group-hover:border-blue-100 transition-all">
                <p class="font-bold text-slate-800"><?= $passer['name'] ?></p>
                <p class="text-[10px] text-blue-600 uppercase font-black tracking-widest mt-1"><?= $passer['program'] ?>
                </p>
