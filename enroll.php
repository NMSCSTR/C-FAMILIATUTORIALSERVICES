...
// Fetch current user's full name from database
$user_query = mysqli_query($conn, "SELECT name FROM users WHERE id = '$user_id'");
$user_data = mysqli_fetch_assoc($user_query);
$enrollee_name = $user_data['name'] ?? 'Valued Student';
                    ...
                    <?php else: ?>
                        ...
                        <form action="" method="POST" class="space-y-8">
                            ...
                            <div>
                                ...
                                <div class="relative">
                                    <select name="batch" required class="w-full p-5 rounded-2xl border-2 border-slate-100 bg-slate-50 focus:bg-white focus:border-blue-600 focus:ring-4 focus:ring-blue-100 outline-none transition font-bold text-slate-700 appearance-none">
                                        <option value="" disabled selected>Select a Batch</option>
                                        <option value="Batch 2026-A (Morning Session)">Batch 2026-A (Morning Session)</option>
                                        <option value="Batch 2026-B (Afternoon Session)">Batch 2026-B (Afternoon Session)</option>
                                        <option value="Intensive Weekend (Sat-Sun)">Intensive Weekend (Sat-Sun)</option>
                                    </select>
